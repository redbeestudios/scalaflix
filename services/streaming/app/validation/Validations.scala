...
trait Validations {
  ...
  def validateGreaterThan(maximumValue: Long)(
    value: Long,
    descriptionPrefix: String = valuePrefix): ValidatedNel[String, Unit] =
    if(value > maximumValue) invalidNel(
      s"$descriptionPrefix$value is greater than $maximumValue."
    ) else validNel(())

  def validateLessThan(minimumValue: Long)(
    value: Long,
    descriptionPrefix: String = valuePrefix): ValidatedNel[String, Unit] =
    if(value < minimumValue) invalidNel(
      s"$descriptionPrefix$value is less than $minimumValue."
    ) else validNel(())

  def validateBetween(minimumValue: Long,
                      maximumValue: Long)(
    value: Long,
    descriptionPrefix: String = valuePrefix): ValidatedNel[String, Unit] =
    validateGreaterThan(minimumValue)(value, descriptionPrefix) |+| validateLessThan(maximumValue)(value, descriptionPrefix)
}
